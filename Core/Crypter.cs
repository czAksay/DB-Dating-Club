...
namespace kursach
{
    ...
    {
        static private int key = 448;

        public static String Encrypt(String inString)
        {
            byte[] byteToEncrypt = System.Text.Encoding.UTF8.GetBytes(inString);
            byte[] byteKey = BitConverter.GetBytes(key);
            byte[] encrypted = encr(byteToEncrypt, byteKey);
            String stxtEnCrypt = System.Text.Encoding.UTF8.GetString(encrypted);
            return stxtEnCrypt;
        }


        private static int[] K;
        private static uint[] s_box = { 0x09d0c479, 0x28c8ffe0, 0x84aa6c39, 0x9dad7287, 0x7dff9be3, 0xd4268361, 0xc96da1d4, 0x7974cc93, 0x85d0582e, 0x2a4b5705,
        ...
        private static int rotl(int val, int pass)
        {
            return (val << pass) | ((int)((uint)val >> (32 - pass)));
        }

        private static int rotr(int val, int pass)
        {
            return ((int)((uint)val >> pass)) | (val << (32 - pass)); ;
        }
        ...
        private static int[] expandKey(byte[] key)
        {
            ...
            {
                for (int i = 0; i < T.Length; i++)
                    T[i] = T[i] ^ (rotl(T[Math.Abs(i - 7 % 15)] ^ T[Math.Abs(i - 2 % 15)], 3) ^ (4 * i * k));
                for (int c = 0; c < 4; c++)
                    for (int i = 0; i < T.Length; i++)
                        T[i] = T[i] + rotl((int)s_box[(int)(T[Math.Abs(i - 1 % 15)] & 0x000001ff)], 9);
                for (int i = 0; i <= 9; i++) tmp[10 * k + i] = T[4 * i % 15];
        ...
        private static long generateMask(long x)
        {
            long m;

            m = (~x ^ ((int)((uint)x >> 1))) & 0x7fffffff;
            m &= (m >> 1) & (m >> 2);
            m &= (m >> 3) & (m >> 6);
        ...
        private static byte[] encryptBloc(byte[] ins)
        {
            byte[] tmp = new byte[ins.Length];
            long aux;

            int[] data = new int[ins.Length / 4];
            for (int i = 0; i < data.Length; i++)
            {
                data[i] = 0;
            }
            ...
            long A = data[0], B = data[1], C = data[2], D = data[3];
            A = A + K[0];
            B = B + K[1];
            C = C + K[2];
            D = D + K[3];
            ...
            {
                B = B ^ s_box[A & 0xff];
                B = B + s_box[(rotr((int)A, 8) & 0xff) + 256];
                C = C + s_box[rotr((int)A, 16) & 0xff];
                D = D ^ s_box[(rotr((int)A, 24) & 0xff) + 256];
            ...
            for (int i = 0; i <= 15; i++)
            {
                ...
                eout = E_func((int)A, K[2 * i + 4], K[2 * i + 5]);
            ...
            for (int i = 0; i <= 7; i++)
            {
                ...
                C = C - s_box[rotr((int)A, 24) & 0xff];
                D = D - s_box[(rotr((int)A, 16) & 0xff) + 256];
                D = D ^ s_box[rotr((int)A, 8) & 0xff];
            ...
            data[0] = (int)A; data[1] = (int)B; data[2] = (int)C; data[3] = (int)D;

            for (int i = 0; i < tmp.Length; i++)
            {
                tmp[i] = (byte)((int)((uint)data[i / 4] >> (i % 4) * 8) & 0xff);
            }

            return tmp;
        }

        private static int[] E_func(int ins, int k1, int k2)
        {
            int[] tmp = new int[3];
            int M, L, R;
            M = ins + k1;
            R = rotl(ins, 13) * k2;
            L = (int)s_box[M & 0x000001ff];
            R = rotl(R, 5);
        ...
        private static byte[] decryptBloc(byte[] ins)
        {
            byte[] tmp = new byte[ins.Length];
            int aux;
            ...
            for (int i = 7; i >= 0; i--)
            {
                ...
                D = (int)(D ^ s_box[(rotr(A, 8) & 0xff)]);
                D = (int)(D + s_box[(rotr(A, 16) & 0xff) + 256]);
                C = (int)(C + s_box[rotr(A, 24) & 0xff]);
                B = (int)(B ^ s_box[(A & 0xff) + 256]);
            ...
            for (int i = 7; i >= 0; i--)
            {
                ...
                D = (int)(D ^ s_box[(rotr(A, 24) & 0xff) + 256]);
                C = (int)(C - s_box[rotr(A, 16) & 0xff]);
                B = (int)(B - s_box[(rotr(A, 8) & 0xff) + 256]);
                B = (int)(B ^ s_box[A & 0xff]);
            }
        ...
        private static byte[] encr(byte[] ins, byte[] key)
        {
            K = expandKey(key);
            int lenght = 0;
            byte[] padding = new byte[1];
            int i;
            lenght = 16 - ins.Length % 16;
            ...
            byte[] tmp = new byte[ins.Length + lenght];
            byte[] bloc = new byte[16];

            int count = 0;

            for (i = 0; i < ins.Length + lenght; i++)
            {
                if (i > 0 && i % 16 == 0)
                {
        ...
        private static byte[] dect(byte[] ins, byte[] key)
        {
            byte[] tmp = new byte[ins.Length];
            byte[] bloc = new byte[16];
            K = expandKey(key);
